def K_Pot(x, y, cx, cy, max_it, R):
    """Computes the continuous potential of a point z = x + iy under p(z) = z^2 + c"""
...
def K_CPM(K, cx, cy, nx, ny, x_min, x_max, y_min, y_max, max_it, R):
    """Computes an approximation of the filled-in Julia set of the quadratic p(z) = z^2 + c
    via the continuous potential method"""
    ...
    # For each pixel in the nx*ny grid, calculate the potential of the point
    for iy in range(0, ny): 
        wy = y_min + iy*(y_max - y_min)/(ny - 1)
        for ix in range(0, nx):
            wx = x_min + ix*(x_max - x_min)/(nx - 1)
            
            # Store the potential in the K array
            K[ix][iy] = K_Pot(wx, wy, cx, cy, max_it, R)
